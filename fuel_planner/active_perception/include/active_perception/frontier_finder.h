...
public:
  ...
  void uncertaintyCallback(const sensor_msgs::PointCloud2::ConstPtr& msg);
  void estimationCallback(const sensor_msgs::PointCloud2::ConstPtr& msg);

  shared_ptr<PerceptionUtils> percep_utils_;
  const std::vector<Eigen::Vector3d>& getUncertaintyCandidates() const {
    return uncandidate_pts_;
  }
...
private:
  ...
  ros::Subscriber rad_sub_;
  ros::Subscriber uncertainty_field_sub_, estimation_field_sub_;
  geometry_msgs::PoseStamped current_pose_;
  double current_radiation_ = 0.0;
  double uncertainty_threshold_ = 0.5;
  std::vector<Eigen::Vector3d> uncandidate_pts_;
  ros::Publisher uncertainty_seeds_pub_;
};
