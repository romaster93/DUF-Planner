...
void FastExplorationManager::initialize(ros::NodeHandle& nh) {
  ...
  exploration_ing_sub_ = nh.subscribe<std_msgs::Bool>("/exploration_ing", 1, &FastExplorationManager::explorationIngCallback, this);
  exploration_end_sub_ = nh.subscribe<std_msgs::Bool>("/exploration_end", 1, &FastExplorationManager::explorationEndCallback, this);
  // Analysis
  // ofstream fout;
...
// 기존 t1 선언 바로 아래에 t2 선언을 추가합니다.
ros::Time t1 = ros::Time::now();
ros::Time t2 = t1;  // <-- 이 줄을 꼭 추가!

// ─── 0) exploration_end_ 처리 ───
if (exploration_end_) {
  // GPR 모드 종료
  dynamic_cube_active_ = false;
  exploration_end_ = false;
  ROS_WARN("Exploration end received → switch back to frontier mode");
}

// ─── 1) GPR 전용 모드 ───
if (dynamic_cube_active_) {
  ros::Time t0 = ros::Time::now();
  // 1-1) GPR로부터 취득한 불확실성 후보만 가져오기
  const auto& uncs = frontier_finder_->getUncertaintyCandidates();
  vector<Vector3d> safe_uncs;
  for (const auto& u : uncs) {
    if (!sdf_map_->isInBox(u)) continue;
    Eigen::Vector3i idx; sdf_map_->posToIndex(u, idx);
    if (sdf_map_->getInflateOccupancy(idx)) continue;
    vector<Vector3d> test_path;
    if (!ViewNode::searchPath(pos, u, test_path)) continue;
    safe_uncs.push_back(u);
  }

  // 1-2) 후보가 없으면 실패
  if (safe_uncs.empty()) {
    ROS_WARN("GPR mode: no valid uncertainty seeds");
    return FAIL;
  }

  // 1-3) 거리 기준 정렬
  std::sort(safe_uncs.begin(), safe_uncs.end(),
    [&](auto& a, auto& b){
      return (a - pos).norm() < (b - pos).norm();
    });

  // 1-4) 첫 번째 시드로 A* + traj
  Vector3d target = safe_uncs.front();
  planner_manager_->path_finder_->reset();
  if (planner_manager_->path_finder_->search(pos, target) == Astar::REACH_END) {
    auto path = planner_manager_->path_finder_->getPath();
    shortenPath(path);
    double time_lb = 0.0;  // yaw 리미트를 무시하거나 필요시 계산
    planner_manager_->planExploreTraj(path, vel, acc, time_lb);
    planner_manager_->planYawExplore(yaw, yaw[0], true, ep_->relax_time_);
    ROS_INFO("GPR mode → moving to seed (%.2f, %.2f, %.2f)  (candidates: %zu, t=%.3fs)",
             target.x(), target.y(), target.z(),
             safe_uncs.size(), (ros::Time::now()-t0).toSec());
    return SUCCEED;
  } else {
    ROS_WARN("GPR mode: A* failed to reach seed");
    return FAIL;
  }
}
...
//   if (has_last_target_) {
//     safe_uncs.erase(
//       std::remove_if(safe_uncs.begin(), safe_uncs.end(),
//         [&](auto& u){ return (u - last_target_).norm() < hysteresis_dist_; }),
//       safe_uncs.end());
//   }
...
//     std::sort(safe_uncs.begin(), safe_uncs.end(),
//       [&](auto& a, auto& b){
//         double da = (a - pos).norm();
//         double db = (b - pos).norm();
//         if (has_last_target_) {
//           da += switch_penalty_ * std::exp(- (a - last_target_).norm());
//           db += switch_penalty_ * std::exp(- (b - last_target_).norm());
//         }
//         return da < db;
//       });

//     // 2-5) 클러스터링
//     const double CLUSTER_R = 1.0;
//     vector<Vector3d> filtered_uncs;
//     for (auto& u : safe_uncs) {
//       bool too_close = false;
//       for (auto& v : filtered_uncs) {
//         if ((u - v).norm() < CLUSTER_R) { too_close = true; break; }
//       }
//       if (!too_close) filtered_uncs.push_back(u);
//     }
...
//     // 2-6) A* → traj → yaw
//     if (!filtered_uncs.empty()) {
//       Vector3d target = filtered_uncs.front();
//       last_target_     = target;
//       has_last_target_ = true;
//       ROS_INFO("UNCERTAINTY PATH START!! target (%.2f, %.2f, %.2f)",
//                target.x(), target.y(), target.z());
...
//         const size_t MIN_WP_COUNT = 3;
//         double len = Astar::pathLength(path);
//         double diff = std::fabs(yaw[0] - std::atan2(target.y()-pos.y(), target.x()-pos.x()));
//         double time_lb = std::min(diff, 2*M_PI-diff) / ViewNode::yd_;
//         const double radius_close = 1.5, radius_far = 5.0;

//         if (path.size() >= MIN_WP_COUNT && len < radius_close) {
//           planner_manager_->planExploreTraj(path, vel, acc, time_lb);
//         } else if (path.size() >= MIN_WP_COUNT && len > radius_far) {
//           vector<Vector3d> truncated;
//           double acc_len = 0;
//           truncated.push_back(path.front());
//           for (size_t i = 1; i < path.size() && acc_len < radius_far; ++i) {
//             acc_len += (path[i] - truncated.back()).norm();
//             truncated.push_back(path[i]);
//           }
//           planner_manager_->planExploreTraj(truncated, vel, acc, time_lb);
//         } else {
//           planner_manager_->kinodynamicReplan(pos, vel, acc, target, Vector3d::Zero(), time_lb);
//         }
...
// ─── 3) 글로벌/로컬 투어 플래닝 이후 로직 (생략 없이 기존 그대로) ───
...
frontier_finder_->getTopViewpointsInfo(pos, ed_->points_, ed_->yaws_, ed_->averages_);
  for (size_t i = 0; i < ed_->points_.size(); ++i) {
    ed_->views_.push_back(
      ed_->points_[i] + 2.0 * Vector3d(std::cos(ed_->yaws_[i]),
                                      std::sin(ed_->yaws_[i]), 0));
  }
  double view_time = (ros::Time::now() - t1).toSec();
  ROS_WARN("Frontier: %zu, t: %lf, viewpoint: %zu, t: %lf",
          ed_->frontiers_.size(), frontier_time,
...
void FastExplorationManager::explorationIngCallback(const std_msgs::Bool::ConstPtr& msg) {
  dynamic_cube_active_ = msg->data;
}

void FastExplorationManager::explorationEndCallback(const std_msgs::Bool::ConstPtr& msg) {
  exploration_end_ = msg->data;
}
...
// 3) 가중치 스케일 & 클램프
const int scale = 1;  // 100에서 1로 낮춤
for (int i = 0; i < dimension; ++i) {
  for (int j = 0; j < dimension; ++j) {
    // raw weight 계산
    long long raw = static_cast<long long>(cost_mat(i, j) * scale + 0.5);
    // INT_MAX/2 이하로 제한
    long long clamped = std::min(raw, static_cast<long long>(INT_MAX/2));
    prob_file << static_cast<int>(clamped) << " ";
  }
  prob_file << "\n";
}
...
while (std::getline(res_file, line)) {
  ...
  if (id == 1) continue;    // 현재 위치(Depot) 무시
  if (id == -1) break;      // 종료 마커
  indices.push_back(id - 2);// solver index → frontier index
}
res_file.close();

// 7) optimal tour 경로로 변환
frontier_finder_->getPathForTour(cur_pos, indices, ed_->global_tour_);
